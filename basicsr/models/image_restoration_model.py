...
class ImageCleanModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('wcg_opt'):
            wcg_type = train_opt['wcg_opt'].pop('type')
            cri_wcg_cls = getattr(loss_module, wcg_type)  
            self.cri_wcg = cri_wcg_cls(**train_opt['wcg_opt']).to(self.device)     
        else:
            raise ValueError('wcg loss are None.')
    ...
    def optimize_parameters(self, current_iter):
        ...
        with autocast(enabled=self.use_amp):
            ...
            l_wcg = 0.
            for pred in preds:
                l_wcg += self.cri_wcg(pred, self.gt) 

            l_total = l_pix+l_wcg

            loss_dict['l_pix'] = l_pix
            loss_dict['l_wcg'] = l_wcg
